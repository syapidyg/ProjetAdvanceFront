...
import { CommandeStockRequestModel } from 'src/app/shared/_models/requests/commande-stock-request.model';

@Component({
  selector: 'app-transfert-stock-article',
  templateUrl: './transfert-stock-article.component.html',
  styleUrls: ['./transfert-stock-article.component.scss']
})
export class TransfertStockArticleComponent implements OnInit {
  ...
  private initForm(data: any) {
    this.form = this.fb.group({
      ...
      idDepotDestination: [data ? data.idDepotDestination : '', Validators.required],
  ...
  ajouterObjet() {
    this.produitService.get(READ_ONE_PRODUIT + '/' + this.f.idProduit.value).then((response: any) => {
      this.depotService.get(READ_ONE_DEPOT + '/' + this.f.idDepot.value).then((responseDepot: any) => {
        this.depotService.get(READ_ONE_DEPOT + '/' + this.f.idDepotDestination.value).then((responseDepotDestination: any) => {
          ...
          const nouvelLigne = {
            ...
            qte: this.qte,
            depotSource: responseDepot.data.name,
            depotDestination: responseDepotDestination.data.name
          };
          const ligneToSave = {
            id: 0,
            qte: this.qte,
  ...
  save() {
    ...
    let dtoRequest;
    dtoRequest = new CommandeStockRequestModel(
      this.f.id.value,
      '',
      this.f.idDepot.value,
      this.f.idDepotDestination.value,
      0,
      'stock',
      '',
      'Transfert',
    ...
    this.commandeService.postTransfert(dtoRequest).toPromise()
      .then((result: any) => {
        console.log('result', result);
        this.isLoading = !this.isLoading;
        this.notif.success('Transfert effectué avec succès ');
        window.location.reload();
      }, err => {
        console.log(err);
        this.notif.danger('Echec lors du transfert ');
        this.isLoading = !this.isLoading;
        this.router.navigate(['/stocks/transfert']);
      });
